...
def get_shift_info(target_date):
    ...
    if remainder in [0, 1]:
        return "AC", "#D4EDDA", "#155724"  # 綠色底, 深綠字
    else:
        return "BD", "#FFF3CD", "#856404"  # 橘色底, 深深橘字

# --- 3. 網頁設定 ---
st.set_page_config(page_title="二休二排班月曆", layout="centered")
st.title("📅 二休二排班大月曆")
...
sel_year = col_y.selectbox("選擇年份", range(2025, 2030), index=1) # 預設 2026
sel_month = col_m.selectbox("選擇月份", range(1, 13), index=date.today().month - 1)
...
# --- 5. 當日值班人員顯示 ---
st.subheader("👥 特定日期人員查詢")
pick_date = st.date_input("查看具體日期", date.today())
on_duty_teams = ['A', 'C'] if get_shift_info(pick_date)[0] == "AC" else ['B', 'D']
...
with st.expander("🛠️ 人員與備註管理"):
    ...
    st.write("---")
    st.write("### 📝 今日備註紀錄")
    user_note = st.text_area("寫點什麼...")
    if st.button("🚀 儲存備註"):
        supabase.table("shift_records").insert({"user_id": "Old_Cha", "shift_date": str(pick_date), "shift_type": "Schedule", "note": user_note}).execute()
        st.success("存好了")
